from extra.sexp import sexp as sexpParser
from extra.util import expr_to_z3, parse_int_variable
import os
from z3 import *
...
def parse_result(result_file, extra=0, with_head=False):
    if not os.path.exists(result_file): return None
    with open(result_file, "r") as inp:
        oup_lines = inp.readlines()
    if len(oup_lines) <= extra: return None
    program = sexp_from_string("".join(oup_lines[extra:]))
    if with_head:
        var_map = {name: i for i, (name, _) in enumerate(program[2])}
        program = program[4]
        return program, var_map
    return program
...
def benchmark_parser(file_name):
    ...
    var_name_list = []
    for _, var_name, var_type in var_list:
        assert var_type == "Int"
