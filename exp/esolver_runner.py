...
esolver_path = "../recommend/esolver"
...
def run_esolver_with_file(file_path, oup_name=None):
    if oup_name is None:
        oup_name = str(random.randint(0, 10 ** 9)) + ".out"
    oup_file = "/tmp/" + oup_name
    command = ["cd", esolver_path, ";", 'ulimit -v ' + str(KMemoryLimit) + ';' + "timeout " + str(KTimeLimit), "./eusolver", file_path, ">", oup_file]
...
def run_esolver_cegis(ps_result, is_cegis=True):
    assert is_cegis
    bm = ps_result["bm"]
    ...
    with open(inp_path, "w") as oup:
        oup.write(bm)
    time_cost, result = run_esolver_with_file(inp_path, oup_name)
    os.system("rm " + inp_path)
    if result is None: return None
    return time_cost, result[0], result[1]

def run_esolver_with_example(p_res, example_space):
    name = str(random.randint(0, 10 ** 9))
    inp_name = name + ".in"
    oup_name = name + ".out"
    inp_path = "/tmp/" + inp_name
...
def run_esolver_random(ps_result, is_cegis = False):
    if is_cegis: return 1 #return run_esolver_cegis(ps_result)
    l = 1
    r = 1
    example_space = []
    g = ps_result["gen"]
    ...
    def evaluate(n):
        ...
        return run_esolver_with_example(ps_result, example_space[:n])
